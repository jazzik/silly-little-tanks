...
exports.a = function () {
    var mapTiles = generate(Math.ceil(globals.l() / 100),
                            Math.ceil(globals.m() / 100));
    ...
    if (globals.h) {
        var collisionTreeObjects = [];
        globals.b.forEach(function (collisionObject) {
            if (collisionObject.type === 'tree') {
                collisionTreeObjects.push(collisionObject);
            }
        });
        collisionTreeObjects.forEach(function (collisionTreeObject) {
            globals.b.splice(globals.b.indexOf(collisionTreeObject), 1);
        });

        for (var i = 0; i < Math.ceil(globals.l() * globals.m() / 125000); i++) {
            generateTree(map);
        }
    }
...
function generateTree(parent) {
    ...
    if (globals.c) {
        while ((globals.c.x - 100 <= randomLeft && globals.c.x + 100 >= randomLeft)
            && (globals.c.y - 100 <= randomTop && globals.c.y + 100 >= randomTop)) {
    ...
    globals.b.push({
        type: 'tree',
        object: {
            gcb: boundaryFn(tree.childNodes[0], 30 * (1.5 - scale))
        }
    });
}
