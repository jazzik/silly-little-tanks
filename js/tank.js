...
var globals = require('./globals'),
    Shell = require('./shell'),
    updateTransform = require('./utils').a,
    collisionDetection = require('./collisionDetection').a,
    getCollisionBoundaryFn = require('./collisionDetection').b,
...
module.exports = function (r, g, b, levels) {
    ...
    self.c = document.createElement('div');

    self.d = function (startPosition, startYaw, onDeadCallback) {
        ...
        self.c.style.position = 'absolute';
        self.c.style.width = '60px';
        self.c.style.height = '60px';
        self.c.style.transform = 'translate3d(0, 0, 0) rotate3d(0, 0, 1, ' + yaw + 'rad)';
        self.c.style.zIndex = '1';
        self.c.style.transition = 'opacity 1s';
        self.c.id = 'tank' + ++id;
        ...
        if (globals.g) {
            cabin.style.boxShadow = '0px 0px 10px 0px #444';
        }
        cabin.style.width = '20px';
        cabin.style.height = '20px';
        cabin.style.background = 'rgb(' + r + ', ' + g + ', ' + b + ')';
        cabin.style.border = '1px solid rgb(' + (r + 40) + ', ' + (g + 40) + ', ' + (b + 40) + ')';
        cabin.style.borderRadius = '10% 10% 40% 40%';
        cabin.style.position = 'absolute';
        cabin.style.left = '20px';
        cabin.style.top = '20px';
        cabin.style.boxSizing = 'border-box';
        cabin.innerHTML = '<div style="width: 10px;height: 25px;background: rgb(' + r + ', ' + g + ', ' + b + ');border: 1px solid rgb(' + (r + 40) + ', ' + (g + 40) + ', ' + (b + 40) + ');position: absolute;left: 4px;bottom: 15px;box-sizing: border-box;"></div>';
        ...
        lifeBar.style.background = 'darkgreen';
        ...
        lifeBarWrapper.appendChild(lifeBar);

        self.c.appendChild(tracks);
        self.c.appendChild(body);
        self.c.appendChild(cabin);
        self.c.appendChild(lifeBarWrapper);

        self.g(startPosition);
    };

    self.e = function () {
        return Math.ceil((levels.cabinYawSpeed + levels.speed + levels.fireTimeout + levels.maxLife + levels.shellDamage) / 5);
    };

    self.f = function (levelName, newValue) {
        switch (levelName) {
            case 'speed':
                forwardSpeed = speedLevels[newValue];
                backwardSpeed = forwardSpeed / 2;
                self.a.speed = newValue;
                break;
            case 'cabinYawSpeed':
                cabinYawSpeed = cabinYawSpeedLevels[newValue];
                self.a.cabinYawSpeed = newValue;
                break;
            case 'fireTimeout':
                fireTimeout = fireTimeoutLevels[newValue];
                self.a.fireTimeout = newValue;
                break;
            case 'maxLife':
                maxLife = maxLifeLevels[newValue];
                self.a.maxLife = newValue;
                break;
            case 'shellDamage':
                shellDamage = shellDamageLevels[newValue];
                self.a.shellDamage = newValue;
                break;
            default:
                break;
        }
    ...
    self.g = function (newPosition) {
        ...
        updateTransform(self.c.style, 'translate3d', renderingPosition.x + 'px, ' + renderingPosition.y + 'px, 0');
    };
    ...
    self.j = function (mouseOnly) {
        ...
        document.addEventListener('mousedown', self.p);
    };

    self.k = function (mouseOnly) {
        if (!playerControlEnabled) {
            return;
        }
        playerControlEnabled = false;
        ...
        document.removeEventListener('mousedown', self.p);
    ...
    self.n = function (shouldBeInFrenzyMode) {
    ...
    self.o = function (damage, forced) {
        if (isAiControlled && !backlog.a.a.fx && !forced) {
            if (!backlog.a.a.k) {
                backlog.b(backlog.a.a);
            }
        ...
        if (life <= 0) {
            ...
            if (globals.k) {
                sounds.k(self.b ? sounds.f : sounds.e);
            }

            if (self.b) {
                setTimeout(function () {
                    alert('You were killed. Hit OK to restart the game');
                    window.location.reload();
                }, 1000);
            }
        } else {
            sounds.k(self.b ? sounds.d : sounds.c);
        }

        updateLifeBar();
    };

    self.gcb = getCollisionBoundaryFn(self.c, 28);

    self.p = function () {
        if (!canFire || globals.j) {
            return;
        }
        ...
        sounds.k(self.b ? sounds.b : sounds.a);
    ...
    function updateLifeBar() {
        ...
        if (lifePercentage === 0) {
            ...
            lifeBar.style.background = 'orange';
    ...
    function renderMovement(dt) {
        ...
        if (controls.forward) {
            self.g({x: position.x + Math.sin(yaw) * forwardSpeed * healthMultiplier * dt, y: position.y - Math.cos(yaw) * forwardSpeed * healthMultiplier * dt});
            ...
            self.g({x: position.x - Math.sin(yaw) * backwardSpeed * healthMultiplier * dt, y: position.y + Math.cos(yaw) * backwardSpeed * healthMultiplier * dt});
    ...
    function generateCloud() {
        var cloud = document.createElement('div'),
            boundaryRect = cabin.getBoundingClientRect();
        ...
        cloud.style.opacity = self.c.style.opacity;
    ...
    function renderRotation(dt) {
        ...
        if (controls.turnLeft) {
            yaw -= dtYawSpeed;
            updateTransform(self.c.style, 'rotate3d', '0, 0, 1, ' + yaw + 'rad');
        }
    ...
    function renderCabin(dt) {
        ...
        if (self.b && !backlog.a.c.fx && lifePercentage <= 30 && globals.k) {
            if (!backlog.a.c.k) {
                backlog.b(backlog.a.c);
            }
            return;
        }

        cabin.style.transform = 'rotate3d(0, 0, 1, ' + (cabinAngle - yaw) + 'rad)';
    }
    ...
    function updateCabinTargetAngle() {
        ...
        if (isAiControlled && globals.c) {
            var myPos = cabin.getBoundingClientRect(),
                dist = Math.sqrt(Math.pow(myPos.left - globals.c.x, 2) + Math.pow(myPos.top - globals.c.y, 2));
            ...
            if (frenzyMode) {
                ...
                targetPosition = globals.c;
                self.p();
            }
        } else if (self.b) {
            targetPosition = globals.a.a;
        }
